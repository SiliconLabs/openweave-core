...
WEAVE_ERROR BLEManagerImpl::_Init()
{
    ...
    ret = bluetooth_start(WEAVE_DEVICE_CONFIG_BLE_LL_TASK_PRIORITY,
                          WEAVE_DEVICE_CONFIG_BLE_STACK_TASK_PRIORITY,
                          initialize_bluetooth);

    VerifyOrExit(ret == WEAVE_NO_ERROR, err = ret);
...
void BLEManagerImpl::bluetoothStackEventHandler(void *p_arg)
{
    ...
    if (flags & BLUETOOTH_EVENT_FLAG_EVT_WAITING) 
    {
      ...
      switch (BGLIB_MSG_ID(bluetooth_evt->header)) 
      {
        ...
        case gecko_evt_gatt_server_characteristic_status_id:
        {
            ...
            StatusFlags = (enum gatt_server_characteristic_status_flag) bluetooth_evt->data.evt_gatt_server_characteristic_status.status_flags;
    ...
    flags = vRaiseEventFlagBasedOnContext(bluetooth_event_flags, 
...
WEAVE_ERROR BLEManagerImpl::ConfigureAdvertisingData(void)
{
  ...
  setAdvDataRsp = gecko_cmd_le_gap_bt5_set_adv_data(WEAVE_ADV_WOBLE_SERVICE_HANDLE,
                                          WEAVE_ADV_SCAN_RESPONSE_DATA,
                                          index,
                                          (uint8_t*) &responseData); 

  if(setAdvDataRsp->result != 0)
  {
      err = setAdvDataRsp->result;
      ExitNow();
  }
...
WEAVE_ERROR BLEManagerImpl::StartAdvertising(void)
{
  ...
  gecko_cmd_le_gap_set_advertise_timing(WEAVE_ADV_WOBLE_SERVICE_HANDLE,
                                        interval_min,
                                        interval_max,
                                        0,
                                        0);

  startAdvRsp = gecko_cmd_le_gap_start_advertising(WEAVE_ADV_WOBLE_SERVICE_HANDLE,
                                      le_gap_user_data, connectableAdv);


  err = startAdvRsp->result;

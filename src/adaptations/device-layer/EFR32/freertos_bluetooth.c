...
void BluetoothTask(void *p)
{
  ...
  while (1) {
    ...
    flags |= xEventGroupWaitBits(   bluetooth_event_flags,   /* The event group being tested. */
                                    ...
                                    xTicksToWait );               /* Wait for maximum duration for bit to be set. With 1 ms tick, portMAX_DELAY will result in wait of 50 days*/

    if (((flags & BLUETOOTH_EVENT_FLAG_STACK) == 0) &&  ((flags & BLUETOOTH_EVENT_FLAG_EVT_HANDLED) == 0) && ((flags & BLUETOOTH_EVENT_FLAG_CMD_WAITING) == 0)){
...
static  void  LinklayerTask(void *p_arg)
{
  ...
  while (1) {
    ...
    uxBits = xEventGroupWaitBits(   bluetooth_event_flags,   /* The event group being tested. */
                                    ...
                                    portMAX_DELAY );         /* Wait for maximum duration for bit to be set. With 1 ms tick, portMAX_DELAY will result in wait of 50 days*/
